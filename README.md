...
```
For convenience, the values are from macros defined in libbcm2835, and they might need to be different if the buttons/LEDs are not physically connected to the default pins in your setup.
...
The default solenoid setup assumes it is connected to the output of a NE555 timer, that generates a fixed time pulse when the recoil event is raised. In this case, a GPIO is connected to the 555 trigger (normally set to HIGH), which goes to logic LOW when the solenoid needs to be fired. This setup is defined in `pigun-gpio.h` as follows:
```C
#define SOL_FIRE 0
#define SOL_HOLD 1
```
...
LED_OK should blink while PiGun is not connected to a host, and go off as soon a connection is established.
...
1. press CAL button (connected to PIN_CAL) - PiGun goes in service mode (LED_CAL goes on)
2. press BT0 (D-pad central button) - switches to the next recoil mode
3. repeat 2 until PiGun is in the desired mode
4. press CAL button - PiGun goes back to idle mode (LED_CAL goes off)
...
In principle the beacons should be as far from each other as possible, in order to get the highest aiming resolution. In practice, the camera as a limited field of view and it needs to see all four beacons when the lightgun is pointed at the corners of the play area.
...
4. the player not in front of the beacons (LEDs have a very narrow emission angle)

The LED_ERR (connected to a red LED) will turn on if the detector routine ends without having found all the four IR spots in the camera image. Each time PiGun enters service mode, the current camera frame is saved in the executable's folder. The file contains the Y channel, one byte for each pixel (416x320). This can be used to check that the beacons are working as intended (also doable with raspivid if the OS is running with X and PiZero is connected to a screen).
...
4. the Wii bar has multiple LEDs for each spot, and some are mounted at an angle, pointing inward and outward so that players do not have to overlap in front of it.
